...
class CycleServiceTest {
    ...
    @BeforeEach
    void setUp() {
        cycleService = new CycleService(cycleRepository, templateRepository, formRepository, userRepository, auditLogService);
    }
    ...
    void triggerCycle_withValidData_shouldSucceed() {
        ...
        when(formRepository.existsByCycleIdAndEmployeeId(anyLong(), anyLong())).thenReturn(false);
        when(userRepository.findById(anyLong())).thenReturn(Optional.of(employee));
        when(formRepository.save(any(AppraisalForm.class))).thenAnswer(i -> i.getArgument(0));
        ...
        verify(formRepository, times(3)).save(any(AppraisalForm.class));
    ...
    @Test
    void triggerCycle_employeeWithNoManager_shouldCountAsFailure() {
        // Arrange
        AppraisalTemplate template = createTemplate(1L);
        AppraisalCycle cycle = createCycle(1L, "Test Cycle", template);
        ...
        assertEquals(10L, result.getFailures().get(0).getEmployeeId());
        verify(formRepository, never()).save(any());
    }

    @Test
    void triggerCycle_existingFormSkipped_shouldCountAsSuccess() {
        // Arrange
        AppraisalTemplate template = createTemplate(1L);
        AppraisalCycle cycle = createCycle(1L, "Test Cycle", template);
        List<Long> employeeIds = List.of(10L);
        ...
        when(templateRepository.findByIsActiveTrue()).thenReturn(Optional.of(template));
        when(cycleRepository.save(any(AppraisalCycle.class))).thenReturn(cycle);
        when(formRepository.existsByCycleIdAndEmployeeId(1L, 10L)).thenReturn(true);
    ...
    void reopenForm_withHRRole_shouldSucceed() {
        ...
        AppraisalTemplate template = createTemplate(1L);
        AppraisalCycle cycle = createCycle(1L, "Test Cycle", template);
        AppraisalForm form = createForm(100L, 1L, 50L, 60L, "SUBMITTED");
        List<String> hrRoles = List.of("HR");

        when(cycleRepository.findById(1L)).thenReturn(Optional.of(cycle));
        when(formRepository.findById(100L)).thenReturn(Optional.of(form));
        when(formRepository.save(any(AppraisalForm.class))).thenReturn(form);

        // Act
        cycleService.reopenForm(1L, 100L, 200L, hrRoles);

        // Assert
        assertEquals("DRAFT_SAVED", form.getStatus());
        assertNull(form.getSubmittedAt());
        assertNull(form.getReviewStartedAt());
        assertNull(form.getReviewedAt());
    ...
    @Test
    void reopenForm_withReviewedAndCompletedStatus_shouldSucceed() {
        AppraisalTemplate template = createTemplate(1L);
        AppraisalCycle cycle = createCycle(1L, "Test Cycle", template);
        AppraisalForm form = createForm(100L, 1L, 50L, 60L, "REVIEWED_AND_COMPLETED");
        List<String> hrRoles = List.of("HR");
    ...
    void reopenForm_withEmptyRolesList_shouldThrowUnauthorizedAccessException() {
        UnauthorizedAccessException exception = assertThrows(UnauthorizedAccessException.class,
                () -> cycleService.reopenForm(1L, 100L, 200L, List.of()));
        assertEquals("Only HR users can reopen forms", exception.getMessage());
    }

    @Test
    void reopenForm_withNonReopenableStatus_shouldThrowValidationException() {
        AppraisalTemplate template = createTemplate(1L);
        AppraisalCycle cycle = createCycle(1L, "Test Cycle", template);
        AppraisalForm form = createForm(100L, 1L, 50L, 60L, "DRAFT_SAVED");
        List<String> hrRoles = List.of("HR");
        ...
        ValidationException exception = assertThrows(ValidationException.class,
                () -> cycleService.reopenForm(1L, 100L, 200L, hrRoles));
        assertTrue(exception.getMessage().contains("DRAFT_SAVED"));
        verify(formRepository, never()).save(any());
    }

    @Test
    void reopenForm_withFormFromDifferentCycle_shouldThrowValidationException() {
        AppraisalTemplate template = createTemplate(1L);
        AppraisalCycle cycle = createCycle(1L, "Test Cycle", template);
        AppraisalForm form = createForm(100L, 99L, 50L, 60L, "SUBMITTED"); // belongs to cycle 99
        List<String> hrRoles = List.of("HR");
    ...
    void reopenForm_withNonExistentCycle_shouldThrowResourceNotFoundException() {
        ...
        assertThrows(ResourceNotFoundException.class,
                () -> cycleService.reopenForm(999L, 100L, 200L, List.of("HR")));
        verify(formRepository, never()).findById(any());
    }
    ...
    void assignBackupReviewer_withValidData_shouldSucceed() {
        ...
        AppraisalCycle cycle = createCycle(1L, "Test Cycle", template);
        AppraisalForm form = createForm(100L, 1L, 50L, 60L, "SUBMITTED");
        User backupReviewer = createUserWithRole(300L, "MANAGER");
        ...
        assertEquals(300L, form.getBackupReviewerId());
        verify(formRepository).save(form);
    ...
    void assignBackupReviewer_withHRRole_shouldSucceed() {
        ...
        AppraisalCycle cycle = createCycle(1L, "Test Cycle", template);
        AppraisalForm form = createForm(100L, 1L, 50L, 60L, "SUBMITTED");
        User backupReviewer = createUserWithRole(300L, "HR");
    ...
    @Test
    void assignBackupReviewer_withEmployeeOnlyRole_shouldThrowValidationException() {
        AppraisalTemplate template = createTemplate(1L);
        AppraisalCycle cycle = createCycle(1L, "Test Cycle", template);
        AppraisalForm form = createForm(100L, 1L, 50L, 60L, "SUBMITTED");
        User backupReviewer = createUserWithRole(300L, "EMPLOYEE");
    ...
    void assignBackupReviewer_withFormFromDifferentCycle_shouldThrowValidationException() {
        AppraisalTemplate template = createTemplate(1L);
        AppraisalCycle cycle = createCycle(1L, "Test Cycle", template);
        AppraisalForm form = createForm(100L, 99L, 50L, 60L, "SUBMITTED"); // cycle 99
    ...
    private AppraisalForm createForm(Long id, Long cycleId, Long employeeId, Long managerId, String status) {
    ...
    private User createUserWithRole(Long id, String roleName) {
        ...
        Set<com.tns.appraisal.auth.Role> roles = new HashSet<>();
        roles.add(role);
        user.setRoles(roles);
        return user;
    }
